...
#define MAX_THREADS 900         // Maximum allowed threads (default and limit)
...
void usage() {
    ...
    printf("If [threads] is not provided, default is %d threads.\n", MAX_THREADS);
    exit(1);
}
...
int main(int argc, char *argv[]) {
    ...
    int threads;
    if (argc == 4) {
        threads = MAX_THREADS; // Default to 900 threads if not provided
    } else {
        threads = atoi(argv[4]);
